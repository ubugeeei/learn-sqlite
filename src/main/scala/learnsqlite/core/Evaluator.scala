...
/** Evaluates scalar expressions using SQLite-like numeric and NULL semantics.
  *
  * See [[https://www.sqlite.org/lang_expr.html Operators and Parse-Affecting Attributes]].
  */
object Evaluator:
  def apply(expression: Expr, schema: Schema, row: Row): Either[String, Value] = expression match
    case Expr.Value(literal)     => Right(fromLiteral(literal))
    case Expr.Column(name)       => schema.resolve(name).map(row(_)).toRight(s"no such column: ${name.value}")
    case Expr.Unary(op, operand) => apply(operand, schema, row).flatMap(unary(op, _))
  ...
  def literal(expression: Expr): Either[String, Value] = expression match
    ...
    case Expr.Unary(UnaryOperator.Negate, operand) => literal(operand).flatMap(unary(UnaryOperator.Negate, _))
  ...
  private def unary(operator: UnaryOperator, value: Value): Either[String, Value] = operator match
    case UnaryOperator.Not => truth(value).map(result => fromTruth(!result))
    case UnaryOperator.Negate => value match
  ...
  private def binary(left: Value, operator: BinaryOperator, right: Value): Either[String, Value] =
    operator match
      ...
      case BinaryOperator.Divide => (left, right) match
  ...
  private def numeric(left: Value, right: Value, ints: (Long, Long) => Long, reals: (Double, Double) => Double) =
    (left, right) match
      case (Value.Null, _) | (_, Value.Null) => Right(Value.Null)
      case (Value.Integer(a), Value.Integer(b)) => Right(Value.Integer(ints(a, b)))
      case (Value.Integer(a), Value.Real(b))    => Right(Value.Real(reals(a.toDouble, b)))
      case (Value.Real(a), Value.Integer(b))    => Right(Value.Real(reals(a, b.toDouble)))
      case (Value.Real(a), Value.Real(b))       => Right(Value.Real(reals(a, b)))
      case _ => Left("arithmetic requires numeric operands")

  private def compare(left: Value, right: Value, predicate: Int => Boolean): Either[String, Value] =
    if left == Value.Null || right == Value.Null then Right(Value.Null)
      ...
      val result = (left, right) match
        ...
        case _                                    => None
      result.map(value => fromTruth(if predicate(value) then Truth.True else Truth.False))
