...
final class Database:
  ...
  private def create(statement: Statement.CreateTable) =
    ...
    tables.get(key) match
      case Some(_) if statement.ifNotExists =>
  ...
  private def select(statement: Statement.Select) =
    table(statement.from).flatMap: target =>
      ...
      val selected = checked.flatMap(_ =>
        target.rows.filterEither(row =>
          matches(statement.where, target.schema, row)
        )
      )
      selected.left
        .map(ExecutionFailure.apply)
  ...
  private def table(name: Identifier): Either[DatabaseError, Table] =
    tables
      .get(name.normalized)
      .toRight(ExecutionFailure(s"no such table: ${name.value}"))
